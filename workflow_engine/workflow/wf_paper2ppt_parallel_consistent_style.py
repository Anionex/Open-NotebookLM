...
from workflow_engine.agentroles import create_react_agent
from workflow_engine.toolkits.multimodaltool.req_img import (
...
def create_paper2ppt_parallel_consistent_graph() -> GenericGraphBuilder:  # noqa: N802
    ...
    async def export_ppt_assets(state: Paper2FigureState) -> Paper2FigureState:
        """
        导出节点：将 ppt_pages 导出为 PDF/PPTX。
        仅当处于「编辑模式且用户未确认全部编辑完成」时跳过，否则一律导出（保证 KB 一次生成后有 PDF 可下）。
        """
        if getattr(state, "gen_down", False):
            all_done = getattr(getattr(state, "request", None), "all_edited_down", False)
            in_edit_flow = getattr(getattr(state, "request", None), "edit_page_num", -1) >= 0
            if in_edit_flow and not all_done:
                return state
