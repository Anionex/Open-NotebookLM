...
2. 节点之间要有明确层级关系
3. **分支均衡性硬约束**：生成前，先规划好所有主分支的标题和预估子节点数量。最大分支的总子节点数不得超过最小分支的 3 倍。如果某个主分支内容特别丰富，必须拆分为 2-3 个更具体的独立主分支，而不是把所有内容堆积在一个分支下
...
4. 层级清晰，从总到分、由主到次
5. 主分支必须恰好 5–8 个，各分支节点数量大致均衡（最大:最小 ≤ 3:1）
6. 覆盖文章所有重要主题板块，不可遗漏文末的总结、展望、哲学思考等内容
7. 不要输出任何解释或额外文字，直接从根节点开始输出："""
