...
4. **剪枝**：如果合并后节点总数超过 40，优先移除 importance_score ≤ 1 的节点
5. **保留关键信息**：importance_score ≥ 4 的节点必须保留，关键数据和定量信息必须保留
6. **重新编号**：node_id 重新编为 "merged_n0", "merged_n1", ...
7. {lang_instruction}
...
## 生成要求
1. 从所有 importance_score=5 的节点中归纳出一个覆盖全文的根节点（# 一级标题）
2. 将 importance_score ≥ 4 的节点组织为主分支（## 二级标题），控制在 4-8 个
3. 将 importance_score ≥ 3 的节点分配为子主题（### 三级标题），每个主分支 2-4 个
4. 更低层级的细节节点按需展开，整体最多 {max_depth} 层
5. 尊重节点间的 parent_topic 层级关系，合理组织树结构
6. 节点命名简洁有信息量，使用短语而非长句
7. 保留关键数据、数字、时间、比例等定量信息
8. 使用{language}语言
