...
from sqlbot_backend.agents.pipeline.state import AgentState, OutputMode, DataFormat, build_initial_state
from sqlbot_backend.agents.pipeline.config import PipelineConfig
from sqlbot_backend.agents.pipeline.graph import build_graph
if os.getenv("SQLBOT_EMBEDDED_MINIMAL", "").strip().lower() in {"1", "true", "yes", "on"}:
    from sqlbot_backend.agents.tools.embedded_registry import get_all_tools
else:
    from sqlbot_backend.agents.tools.registry import get_all_tools
from sqlbot_backend.core.config import settings
from sqlbot_backend.core.llm_factory import LLMFactory, LLMConfig

# Initialize RAG services (side effects: registers examples)
from sqlbot_backend.modules.rag.few_shot import init_standard_examples  # noqa: F401
...
class SQLBotAgent:
    ...
    async def run(
        ...
        try:
            ...
            from sqlbot_backend.agents.tools.cross_source_tools import close_unified_engine
