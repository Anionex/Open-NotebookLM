...
from sqlbot_backend.agents.pipeline.nodes.ega_prepare import should_run_ega_prepare
...
def query_understanding_node(state: AgentState, config: PipelineConfig) -> dict:
    ...
    if config.rag_enable_query_rewrite:
        try:
            from sqlbot_backend.modules.rag.query_rewrite import query_rewrite_service, RewriteResult
    ...
    if config.rag_enable_analysis_cot and not ega_lite_mode:
        ...
        if any(k in (question or "") for k in analysis_keywords):
            try:
                from sqlbot_backend.modules.rag.analysis_cot import analysis_cot_service, AnalysisCoTResult
